function data = FILA_anaList(filePos,ana_mode,scale, thresh)
...
%
% GETS:
%        filePos = return value of getAllFilesLinux
...
switch ana_mode,
    ...
    case 'full_none',
        ...
        data = struct('image',[],'outterBoundary',[],'centroid',[],...
            ...
            'searchAreas',[],'longLum',[]);
        data = repmat(data,length(filePos),1);
        %load ana save
        for imageI =1: length(filePos),
    ...
    case 'full_plot',
        ...
        for imageI =1: length(filePos),
            imageL = imread(filePos{imageI});
            data(imageI) =  FILA_fullImageAnalysis(imageL,scale,thresh);
            figure(h),clf
    ...
    case 'light'
        ...
        listLen = length(filePos);
        data = repmat(data,listLen,1);
        for imageI =1: listLen,
            if rem(imageI,10) ==0,
                disp([' anaylsed: ' num2str(imageI) ' of ' num2str(listLen) ' images'])
            end
            %load ana light
            imageL = imread(filePos{imageI});
