...
%crop image
[cropImage,outterBoundary] = FILA_imcrop2boundary(imageRot,outterBoundary,thresh);
% [B,A] = butter(2,0.5);
% outterBoundary   = filtfilt(B,A,outterBoundary);
...
% figure(2);imagesc(cropImage);colormap(gray);hold on ; 
% plot(outterBoundary(:,2),outterBoundary(:,1),'g--');plot(spline(:,2),spline(:,1),'b--'); hold off
% axis equal
% axis tight
% drawnow
