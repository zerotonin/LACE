function traceResult = ET_anaS_basic4splineMov(movObj,...
...
%                       % footage
% multiDetectionPossible = 0;% in a benzer movie flies cannot chain,
%                            %  therfore this is set to zero
% 
% % loop through all frames
% for frameNumber =1:headerInfo.AllocatedFrames,
%     % run detection script
%     traceResult{frameNumber} = ET_anaS_basic4singleFrame(fid,headerInfo,endianType,...
%         IDX,frameNumber,angleCorrection,backGround,bgSubModus,contrastThresh,eroderR,...
%         ellipseParams,bestEllipsesN,prevDetection,expetedLength,expetedSize,voteWeights,...
%         expectedAnimals,multiDetectionPossible);
%     % if possible use last detection for prev detection
%     if size(traceResult{frameNumber},1) == expectedAnimals,
%         prevDetection = traceResult{frameNumber}(:,1:2);
%     else
%         prevDetection =[];
%     end
% end
%
%
% see also  ivT_norpix_loadSingleImage, ET_im_rotate, ET_im_diffImage, 
% ET_im_normImage,ET_im_diffImage,ET_im_erode,ET_HTD_detectObjectContours,
% ET_HTD_splitImage2ellipseImages,ET_HTD_detectEllipses,
% ET_HTD_splitCoords2ImageCoords, ET_eva_ellipseTests, ET_ac_solveStandardProblems
% ET_ac_classifyProblem
...
% rotate image
imageBin = ET_im_rotate(imageBin,angleCorrection);
% use ROI
if ~isempty(maskIDX),
...
if max(animalNumber) == 1 && sum(animalNumber) == expectedAnimals,
    ...
    % get spline
    spline = cellfun(@(x) FILA_ana_getSpine(filtfilt(B,A,x)),objectContours(logIDX),'UniformOutput',false );
    %get head positions
    [hP,tP] = cellfun(@(x,y) ET_phA_findFishHead(x,y),objectContours(logIDX),spline,'UniformOutput',false);
    %spline = cellfun(@(x,y) FILA_binSpline(x,y),spline,objectContours(logIDX),'UniformOutput',false );
    ...
    traceResult = {[traceResult  animalNumber(logIDX) evaluation(logIDX,[1:4 6]) zeros(size(traceResult,1),1)],...
        objectContours(logIDX), spline, cell2mat(hP), cell2mat(tP)};
else
    %%%%%%%%%%%%%%%%%%%
    % AUTO CORRECTION %
    %%%%%%%%%%%%%%%%%%%
