function trigAve = triggeredAverageND(data,trigInd,winSize,indices,set2zero)
...
%
% RETURNS:
%    trigAve = n x win*2+1 x 3 matrix containing the averaged data. Note 
...
%
% SYNTAX: trigAve = triggeredAverageND(data,trigInd,winSize,indices);
%
...
for j =1:numFeatures,
    ...
    if set2zero,
       temp = bsxfun(@minus,temp,temp(:,1));
    end
    % write the  median and confidence intervals
    trigAve(j,:,1) = median(temp,1);
    [trigAve(j,:,2),trigAve(j,:,3)] = confintND(temp,1);
end
