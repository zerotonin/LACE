function expectedAnimals = ET_phA_mostOftenFoundNoAnimals(traceResult)
...
% get all unique amounts of found animals
uniqNumbers = unique(foundAnimals);
% now count which was found how often
countFrames = NaN(length(uniqNumbers),1);
for i =1:length(uniqNumbers),
    countFrames(i) = sum(foundAnimals == uniqNumbers(i));
end
% pick the one that was found most often and define it to be the expected
% animal number
[~,i]=max(countFrames);
expectedAnimals =uniqNumbers(i);