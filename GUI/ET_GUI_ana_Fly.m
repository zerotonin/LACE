function varargout = ET_GUI_ana_Fly(varargin)
% ET_GUI_ANA_FLY MATLAB code for ET_GUI_ana_Fly.fig
%      ET_GUI_ANA_FLY, by itself, creates a new ET_GUI_ANA_FLY or raises the existing
%      singleton*.
%
%      H = ET_GUI_ANA_FLY returns the handle to a new ET_GUI_ANA_FLY or the handle to
%      the existing singleton*.
%
%      ET_GUI_ANA_FLY('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ET_GUI_ANA_FLY.M with the given input arguments.
%
%      ET_GUI_ANA_FLY('Property','Value',...) creates a new ET_GUI_ANA_FLY or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ET_GUI_ana_Fly_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ET_GUI_ana_Fly_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ET_GUI_ana_Fly

% Last Modified by GUIDE v2.5 03-May-2016 11:24:38

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @ET_GUI_ana_Fly_OpeningFcn, ...
                   'gui_OutputFcn',  @ET_GUI_ana_Fly_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before ET_GUI_ana_Fly is made visible.
function ET_GUI_ana_Fly_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ET_GUI_ana_Fly (see VARARGIN)

% Choose default command line output for ET_GUI_ana_Fly
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ET_GUI_ana_Fly wait for user response (see UIRESUME)
...
setappdata(handles.output,'fps',500);
setappdata(handles.output,'expectedAnimals',1);
setappdata(handles.output,'saccThresh',200);
...
% set strains PopMenu 
strains = ET_SR_getStandardStrains;
strains = strains(:,1);
...
function varargout = ET_GUI_ana_Fly_OutputFcn(hObject, eventdata, handles)
...
if isempty(getappdata(handles.output,'movieFilePos')),
    ...
    if FileName ~=0,
        ...
        analyseTraceResult(hObject, eventdata, handles,1);
...
if sum(handleMatch)>0,
    newSliderPos = x(1)/(movieFPos.headerInfo.AllocatedFrames/fps);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
analyseTraceResult(hObject, eventdata, handles,0);
refreshPlots(hObject, eventdata, handles);
end

% --- Executes on button press in pb_flipDir.
function pb_flipDir_Callback(hObject, eventdata, handles)
% hObject    handle to pb_flipDir (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% frameNumber = sliderPos2frameNumber(hObject, eventdata, handles);
% prompt = {'Enter frame from which to flip orientation:', 'stop flipping:'};
% dlg_title = 'Flip da fly';
% num_lines = 1;
% def = {num2str(frameNumber),num2str(length(traceResult))};
% answer = inputdlg(prompt,dlg_title,num_lines,def);
% if ~isempty(answer),
%     answer = cellfun(@str2double,answer);
    %flip flys
    traceResult(:,1) = ...
                          cellfun(@(x) [x(:,1:4) x(:,5)+180 x(:,6:end)],...
                    traceResult(:,1),'UniformOutput',false);
    
    setappdata(handles.output,'traceResult',traceResult);
    % recalculate
    analyseTraceResult(hObject, eventdata, handles,0,1);
    refreshPlots(hObject, eventdata, handles);
% end
end
...
traceResult = getappdata(handles.output,'traceResult');
fid = movieFpos.fid;
endianType = movieFpos.endianType;
headerInfo = movieFpos.headerInfo;
IDX = movieFpos.IDX;
fPos = movieFpos.fPos;
frameNumber = sliderPos2frameNumber(hObject, eventdata, handles);
setappdata(handles.output,'currentFrame',frameNumber);
titleStr = ['frame # ' num2str(frameNumber) ' | ' fPos];

frame = ivT_norpix_loadSingleImage(fid,headerInfo,endianType,IDX',frameNumber);
...
plotTracing(handles.axes5,traceResult,frameNumber,handles)
...
plotTracing(handles.axes10,traceResult,frameNumber,handles)
end
...
movieFpos = getappdata(handles.output,'movieFilePos');
headerInfo = movieFpos.headerInfo;
frameNumber = round(sliderPos*length(movieFpos.IDX))+1;
if frameNumber > headerInfo.AllocatedFrames,
    frameNumber = headerInfo.AllocatedFrames;
end
end
...
headerInfo = movieFpos.headerInfo;
sliderPos = frameNumber/headerInfo.AllocatedFrames;
end
...
hold(axH,'on')
plot(axH,xAx([1 end]),zeros(1,2),'--','Color',[0.5 0.5 0.5])
hold(axH,'off')
...
function plotTracing(axH,traceResult,frameNumber,handles)
flyContour = traceResult{frameNumber,2};
flyContour = [flyContour{:}];
traceR = traceResult{frameNumber,1};
tailLength= round((abs(diff(get(axH,'XLim')))+abs(diff(get(axH,'YLim'))))/20);
trace = getappdata(handles.output,'trace');
hold(axH,'on')
plot(axH,flyContour(:,2),flyContour(:,1),'g');
ET_plot_addEllipse(traceR,'gray',80,axH,0,1);
ET_plot_lollipop(traceR(1,1,1),traceR(1,2,1),trace(frameNumber,3,1),tailLength,[1 0 0],axH);
...
ET_plot_lollipopTrace(trace(:,[2 1 3]),fps,round(size(trace,1)/20),1,0,tailLength,handles.axes11)
axis(handles.axes11,'image')
hold(handles.axes11,'on')
ET_plot_lollipop(trace(frameNumber,2,1),trace(frameNumber,1,1),trace(frameNumber,3,1),tailLength,[1 0 0],handles.axes11);
...
function analyseTraceResult(hObject, eventdata, handles,firstRun,yawCorr)
...
pix2mm = getappdata(handles.output,'pix2mm');
[trace,traceResult,newIDX] = ET_anaS_phAnaFly(traceResult,fps,expectedAnimals,butterDeg,butterCo,pix2mm,yawCorr);
[saccs, ~] = get_saccades(trace(startF:lastF,6),saccThresh);
saccs(:,1:3)=saccs(:,1:3)+startF-1;
trigAveL = triggeredAverage([rad2deg(trace(:,3)),trace(:,6)],saccs(saccs(:,4) == 1,2),round(fps/15),[],10);
trigAveR = triggeredAverage([rad2deg(trace(:,3)),trace(:,6)],saccs(saccs(:,4) == -1,2),round(fps/15),[],10);
...
bendability = bendability2(saccI);
