...
if goOn==1,
    ...
    fprintf(fid,'%s\n','% open file dialogue');
    if sortNorpix ==1,
        fprintf(fid,'%s\n',['[fid, endianType,headerInfo,imgOut, ~] = ivT_norpix_openSeq(''' movieFpos.fPos ''',''onlyHeader'',''sort'');']);
    else
        fprintf(fid,'%s\n',['[fid, endianType,headerInfo,imgOut, ~] = ivT_norpix_openSeq(''' movieFpos.fPos ''',''onlyHeader'','''');']);
    end
    ...
    if prevDetection ==1,
        ...
        fprintf(fid,'%s\n','traceResult(1,:) = ET_anaS_basic4Benzer(fid,headerInfo,endianType,...');
        ...
        fprintf(fid,'%s\n','         traceResult(frameNumber,:) = ET_anaS_basic4Benzer(fid,headerInfo,endianType,...');
