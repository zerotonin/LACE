function ET_GUI_Benzer_SR_writeScript(handles)
...
%get movie position
movFileName = movieFpos.fPos{1};
% get file basis name
[fileBasis,~,~]=fileparts(movFileName);
[~,fileBasis,~]=fileparts(fileBasis);
...
if goOn==1,
    ...
    [bgFileName,roiFileName,scriptFileName,variablesFileName,...
             resultFileName] = ET_GUI_spline_SR_autoNamingScripts(handles,fileBasis);
    else
...
if goOn==1,
    ...
    save(variablesFileName,...
        ...
        'expetedSize','voteWeights','expectedAnimals','multiDetectionPossible',...
        'pix2mm','distortParams');
    
    fid = fopen(scriptFileName,'w');
    
    % Header
    fprintf(fid,'%s\n','% This script was automatically designed by ET_GUI_Benzer for further information');
    ...
    if prevDetection ==1,
        %central loop
        fprintf(fid,'%s\n','% central loop');
        fprintf(fid,'%s\n','traceResult= cell(length(movieFpos.IDX),6);');
        fprintf(fid,'%s\n','traceResult(1,:) = ET_anaS_basic4BenzerStack(movieFpos.fPos,...');
        fprintf(fid,'%s\n','  movieFpos.IDX,1,angleCorrection,backGround,bgSubModus,contrastThresh,eroderR,...');
        ...
        fprintf(fid,'%s\n','         traceResult(frameNumber,:) = ET_anaS_basic4BenzerStack(movieFpos.fPos,...');
