function varargout = ET_GUI_Spline(varargin)
% ET_GUI_SPLINE MATLAB code for ET_GUI_Spline.fig
%      ET_GUI_SPLINE, by itself, creates a new ET_GUI_SPLINE or raises the existing
%      singleton*.
%
%      H = ET_GUI_SPLINE returns the handle to a new ET_GUI_SPLINE or the handle to
%      the existing singleton*.
%
%      ET_GUI_SPLINE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ET_GUI_SPLINE.M with the given input arguments.
%
%      ET_GUI_SPLINE('Property','Value',...) creates a new ET_GUI_SPLINE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ET_GUI_Spline_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to ET_GUI_Spline_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ET_GUI_Spline

% Last Modified by GUIDE v2.5 06-Jul-2016 09:30:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @ET_GUI_Spline_OpeningFcn, ...
    'gui_OutputFcn',  @ET_GUI_Spline_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before ET_GUI_Spline is made visible.
function ET_GUI_Spline_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to ET_GUI_Spline (see VARARGIN)

% Choose default command line output for ET_GUI_Spline
...
setappdata(handles.output,'conThresh', 0.25);
...
setappdata(handles.output,'expAnimalLen', 290);
...
% UIWAIT makes ET_GUI_Spline wait for user response (see UIRESUME)
% uiwait(handles.figure1);
end


% --- Outputs from this function are returned to the command line.
function varargout = ET_GUI_Spline_OutputFcn(hObject, eventdata, handles)
...
if goOn ==1
    % clear old variables
    ET_GUI_Spline_SR_clearHandles(handles);
...
% check if ET_scripts exists

folder_name = getappdata(handles.output,'detScriptPath');
...
if goOn ==1,
        ...
        case 'norpix'
            ET_GUI_spline_SR_writeScript(handles)
        case 'videoReader'
            ET_GUI_spline_SR_writeScriptMov(handles);
        otherwise
...
% --- Executes on button press in pb_loadPref.
function pb_loadPref_Callback(hObject, eventdata, handles)
% hObject    handle to pb_loadPref (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

preferences =load2var([getappdata(handles.output,'detScriptPath') 'ET_Pref_Manager.mat']);
listCell = cellfun(@(x,y) [x '     ' datestr(y)],preferences(:,1),preferences(:,3),'UniformOutput',false );

[s,v] = listdlg('PromptString','Select a file:',...
    'SelectionMode','single',...
    'ListString',listCell,'ListSize',[300 300],'fus',10,'ffs',18);
if v ==1,
    newPref = preferences{s,2};
    names = fieldnames(newPref);
    for i =4:length(names),%first three are GUI intern
        eval(['setappdata(handles.output,''' names{i} ''',newPref.' names{i} ' );']);
    end
end
...
% --- Executes on button press in pb_savePref.
function pb_savePref_Callback(hObject, eventdata, handles)
% hObject    handle to pb_savePref (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
newPref = getappdata(handles.output);
name = inputdlg('Name of the prefernce profile: ','Save prefernces', [1 25]);
preferences =load2var([getappdata(handles.output,'detScriptPath') 'ET_Pref_Manager.mat']);
if isempty(preferences),
    preferences = [name {newPref} {now}];
else
    %check if the same name was used before
    idx = find(~cellfun(@isempty,strfind(preferences(:,1),name{1})));
    if isempty(idx),
        preferences = [preferences;name {newPref} {now}];
    else
        % Construct a questdlg with three options
        choice = questdlg({'There is allready a preference profile with this name',...
            [ 'from the ' datestr(preferences{idx,3}) ' !']},...
            'Name error', ...
            'Overwrite','Abort','Abort');
        % Handle response
        if strcmp(choice,'Overwrite')
            preferences(idx,2:3) = [{newPref} {now}];
        end
    end
end
save([getappdata(handles.output,'detScriptPath') 'ET_Pref_Manager.mat'],'preferences');
end
...
for i =frameIDX,
    ...
    ET_GUI_spline_SR_plotTraceResult(handles,frame,traceResult,['frame: ' num2str(i) ...
...
frame = ivT_norpix_loadSingleImage(movieFpos.fid,movieFpos.headerInfo,movieFpos.endianType,movieFpos.IDX,frameNumber,getappdata(handles.output,'headerSize'));
frame= double(frame);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    stru cla(handles.axes1)cture with handles and user data (see GUIDATA)
setappdata(handles.output,'meanBG', []);
end
...
function ET_GUI_spline_SR_plotTraceResult(handles,img,traceResult,titleStr)
oc = traceResult{1,2};
centralLine = traceResult{1,3};
head = traceResult{1,4};
tail = traceResult{1,5};
...
plot(handles.axes1,head(:,2),head(:,1),'bo','MarkerSize',10,'LineWidth',2)
plot(handles.axes1,tail(:,2),tail(:,1),'b>','MarkerSize',10,'LineWidth',2)
hold off
end
...
if isempty(bgSubModus) || strcmp(bgSubModus,'difference direction'),
    ...
    traceResult = ET_anaS_basic4spline(movieFpos.fid,movieFpos.headerInfo,movieFpos.endianType,...
        ...
        expectedAnimals,multiDetectionPossible,maskIDX,getappdata(handles.output,'headerSize'));
        case 'videoReader'
    traceResult = ET_anaS_basic4splineMov(movieFpos.movObj,movieFpos.IDX,frameNumber,...
        angleCorrection,backGround,bgSubModus,contrastThresh,eroderR,...
        ellipseParams,bestEllipsesN,prevDetection,expetedLength,expetedSize,voteWeights,...
        expectedAnimals,multiDetectionPossible,maskIDX);
        otherwise
            error(['ET_GUI_spline,line 780: Unknown file format for image source: ' getappdata(handles.output,'movieIOtype')])
    end
...
function ET_GUI_Spline_SR_clearHandles(handles)
%Initialise old variables
% Image I/O
setappdata(handles.output,'movieIOtype', '');
setappdata(handles.output,'sortNorpix', 0);
