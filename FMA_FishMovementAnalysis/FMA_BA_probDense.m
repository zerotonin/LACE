function [bendSweetSpot,bendSweetSpot2,bendSweetSpotMat,data,data2] = FMA_BA_probDense(ROI,thrustThresh,analysedData)

% ROI
[in,on] = inpolygon(analysedData.trace(:,2),analysedData.trace(:,1),ROI(:,1),ROI(:,2));
% categoriser
[~,~,idxS,~] = FMA_manCategoriser( analysedData.trace, analysedData.saccs,thrustThresh);
%combine
logIDX = ~idxS & (in | on);
if sum(logIDX) ~=0
    ...
    data2 = bsxfun(@plus,data(:,1:15),fliplr(data(:,16:30)));
else
    bendSweetSpot=NaN;
    bendSweetSpot2=NaN;
