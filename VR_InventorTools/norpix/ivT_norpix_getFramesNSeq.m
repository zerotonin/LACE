function [headerInfo,imgOut] = ivT_norpix_getFramesNSeq(fid,endianType,headerInfo)
% This function of the ivT norpix IO toolbox (ivT_norpix) loads the image
% sequence into a single matrix (imgOut). headerInfo will be updated with
% the timestamp field.
%
% GETS:
...
%              imageWidth, p number of frames
%
% SYNTAX:[headerInfo,imgOut] = ivT_norpix_getFramesNSeq(fid,endianType,headerInfo);
%
% EXAMPLES:
...
%
% see also ivT_norpix_loadSingleImage, ivT_norpix_sortSeq,ivT_norpix_getTimeStamps
...
for i =1:headerInfo.AllocatedFrames,  
    ...
    fseek(fid, 1024 + (nread-1) * (headerInfo.TrueImageSize), 'bof');
    %fseek(fifseek(fid,-8,'cof')d, 1024 + nread * (headerInfo.ImageSizeBytes+8), 'bof');
    ...
    %write out images if desired (can only work if movie fits into RAM)
    if nargout > 1
        imgOut(:,:,nread) = permute(reshape(tmp,imSize(1),imSize(2),[]),[2,1,3]);
    end
