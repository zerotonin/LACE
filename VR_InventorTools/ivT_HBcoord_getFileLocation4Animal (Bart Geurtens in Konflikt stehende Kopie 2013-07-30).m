 function [fileList, frames] = ivT_HBcoord_getFileLocation4Animal()
...
% 
% GETS:
%         nothing input is handled in verbose mode.
%
% RETURNS:
...
while ischar(infoLine),
    ...
    if length(doneNb) == 2,
        candidateDirectoryName = [candidateDirectoryName; infoLine(1:20)];
        frames = [frames; sscanf(infoLine(21:doneNb(1)-1),'%d-%d')'];
    end
    infoLine = fgetl(fid);
end
fclose(fid);
