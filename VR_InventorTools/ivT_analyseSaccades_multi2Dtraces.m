function [trigAve,saccs] = ivT_analyseSaccades_multi2Dtraces(traces,threshold,winSize,set2zero,modus)
% This function analysis the yaw velocity of multiple 2D ivTrace trajectories.
% Therefore cellfun is employed to use the get_saccades function on all
...
% 
% RETURNS
%           saccs = A cell array of the length of traces. Every cell holds
%                   a mxn matrix where m is the number of found saccades
...
%                   intervals are given.
%         trigAve = A cell array of the length of traces. Every cell holds
%                   a nxwin*2+1x3 matrix containing (if modus = 'median')
...
%
% SYNTAX: [trigAve,saccs] = ivT_analyseSaccades_multi2Dtraces...
...
% get saccades
saccs = cellfun(@(x) get_saccades(x(:,8),threshold),traces,'UniformOutput',false);
% calculate triggered average
switch modus
    case 'median'
    trigAve =cellfun(@(x,y) triggeredAverageND(x(:,6:8),y(:,2),winSize,1:size(x,1),set2zero),traces,saccs,'UniformOutput',false);
    case 'mean'
    trigAve =cellfun(@(x,y) triggeredAverage(x(:,6:8),y(:,2),winSize,1:size(x,1),set2zero),traces,saccs,'UniformOutput',false);
    otherwise
